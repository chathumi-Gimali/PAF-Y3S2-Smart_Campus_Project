...
export default function CreateBooking({ user, setActivePage }) {
  ...
  const validate = () => {
    if (needsJustification) {
      if (parseInt(form.expectedAttendees) < 10) {
        return "Students need at least 10 attendees to book lecture halls, labs, or auditorium.";
      }
      if (!form.reason.trim()) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const purpose = needsJustification ? `${form.purpose} | Reason: ${form.reason}` : form.purpose;
      ...
      setMessage("Booking request submitted! Waiting for admin approval.");
  ...
  const inputStyle = { width: "100%", padding: "10px 14px", borderRadius: "8px", border: "1.5px solid #e0e0e0", fontSize: "14px", outline: "none" };
  const labelStyle = { fontSize: "13px", fontWeight: "600", color: "#444", marginBottom: "6px", display: "block" };

  const availableResources = isStudent
    ? RESOURCES // students can see all but will be validated
    : RESOURCES;
    ...
    <div>
      ...
      {isStudent && (
        <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: "12px", padding: "14px 18px", marginBottom: "20px" }}>
          <div style={{ fontWeight: "600", color: "#1e40af", marginBottom: "4px" }}>📌 Student Booking Policy</div>
          <div style={{ fontSize: "13px", color: "#1e40af" }}>
            You can freely book <b>Study Rooms</b> and <b>Meeting Rooms</b>.<br />
            To book <b>Lecture Halls, Labs, or Auditorium</b> you need: minimum 10 attendees + specific reason.
          </div>
        </div>
      )}
      ...
      <div style={{ background: "white", borderRadius: "16px", padding: "32px", maxWidth: "620px", boxShadow: "0 2px 12px rgba(37,99,235,0.08)" }}>
        ...
        {error && <div style={{ background: "#fee2e2", color: "#991b1b", padding: "12px 16px", borderRadius: "8px", marginBottom: "20px", fontWeight: "600" }}>❌ {error}</div>}
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: "20px" }}>
            ...
            <select style={{ ...inputStyle, background: "white" }}
              ...
              <optgroup label="📚 Study Areas">
                {RESOURCES.filter(r => r.type === "STUDY_AREA").map(r => <option key={r.id} value={r.id}>{r.label}</option>)}
              </optgroup>
              <optgroup label="🤝 Meeting Rooms">
                {RESOURCES.filter(r => r.type === "MEETING_ROOM").map(r => <option key={r.id} value={r.id}>{r.label}</option>)}
              </optgroup>
              <optgroup label="🏫 Lecture Halls {isStudent ? '(Restricted)' : ''}">
                {RESOURCES.filter(r => r.type === "LECTURE_HALL").map(r => <option key={r.id} value={r.id}>{r.label} {isStudent ? "⚠️" : ""}</option>)}
              </optgroup>
              <optgroup label="🔬 Labs">
                {RESOURCES.filter(r => r.type === "LAB").map(r => <option key={r.id} value={r.id}>{r.label} {isStudent ? "⚠️" : ""}</option>)}
              </optgroup>
              <optgroup label="🎭 Auditorium">
                {RESOURCES.filter(r => r.type === "AUDITORIUM").map(r => <option key={r.id} value={r.id}>{r.label} {isStudent ? "⚠️" : ""}</option>)}
              </optgroup>
            </select>
          </div>
          ...
          {needsJustification && (
            <div style={{ background: "#fffbeb", border: "1px solid #fcd34d", borderRadius: "8px", padding: "12px 16px", marginBottom: "16px" }}>
              <div style={{ fontSize: "13px", color: "#92400e", fontWeight: "600" }}>⚠️ Restricted Facility — Requires Justification</div>
              <div style={{ fontSize: "12px", color: "#92400e", marginTop: "4px" }}>Minimum 10 attendees and a specific reason are required.</div>
            </div>
          )}

          <div style={{ marginBottom: "20px" }}>
            <label style={labelStyle}>Purpose</label>
            <input style={inputStyle} placeholder="e.g. Group study, Workshop, Project presentation"
              value={form.purpose} onChange={e => setForm({ ...form, purpose: e.target.value })} required />
          </div>

          <div style={{ marginBottom: "20px" }}>
            <label style={labelStyle}>Expected Attendees {needsJustification && <span style={{ color: "#ef4444" }}>(min. 10)</span>}</label>
            <input style={inputStyle} type="number" min={needsJustification ? 10 : 1}
              value={form.expectedAttendees} onChange={e => setForm({ ...form, expectedAttendees: e.target.value })} required />
          </div>

          {/* Extra reason for students booking restricted */}
          {needsJustification && (
            <div style={{ marginBottom: "20px" }}>
          ...
          }}>{loading ? "Submitting..." : "Submit Booking Request"}</button>
        </form>
      </div>
    </div>
  );
